...
def fetch_start_time(df1,df2,status):
    ...
    #selecting the LIMS test which have status as "In Progress"
    iptests = status[status["status"]=="In Progress"]

    #merginf with the LIMS data and fetching the earliest timestamps
    iptests = iptests.merge(df,on=["batchuniqueidentifier","analysis_common_name"], how="inner")
    iptests['test_submitted_date'] = pd.to_datetime(iptests['test_submitted_date'])
    iptestdate = iptests.groupby(["batchuniqueidentifier","analysis_common_name"])['test_submitted_date'].min().reset_index()

    status = status.merge(iptestdate,on=["batchuniqueidentifier","analysis_common_name"],how="left")

    status_map = {"Completed":9,"In Progress":5}
    status['status_map'] = status['status'].map(status_map)

    status = status[['batchuniqueidentifier','analysis_common_name', 'status_map', 'qc_status', 'test_submitted_date']]

    status["qc_status"] = status["qc_status"].astype("int64")
    print("Total Tests where LIMS status not equal to QC : ",status[status["status_map"]!=status["qc_status"]].shape[0])

    return status[status["status_map"]!=status["qc_status"]]
